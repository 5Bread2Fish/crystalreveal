...
export default function AdminPage() {
    ...
    const fetchStats = async () => {
        ...
        try {
            ...
            if (data.history) {
                // Determine source: Database returns full history, sort by timestamp desc just in case
                const sorted = data.history.sort((a: Log, b: Log) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
                setLogs(sorted);
    ...
    const exportCSV = () => {
        ...
        link.setAttribute("download", `bomee_db_${new Date().toISOString()}.csv`);
